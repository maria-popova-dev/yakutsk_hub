...
Widget build(BuildContext context) {
  int yktHour = AppUtils.getYakutskTime().hour;
  String status = (yktHour >= 16 || yktHour < 6)
      ? "ЗАНЯТИЯ ОКОНЧЕНЫ"
      : _getActivatedStatus();
...
title: Column(children: [
  Text('ЯКУТСК • HUB',
      style: GoogleFonts.inter(letterSpacing: 3, fontWeight: FontWeight.w900, fontSize: 18, color: Colors.white)),
...
Expanded(child: HubWidgets.buildSmallCard(
    'ПАРОМЫ',
    'YKT LIVE', Icons.directions_boat, const Color(0xFFFF9500),
() => Navigator.push(context, MaterialPageRoute(builder: (c) => const SummaryScreen())))),
]),
const SizedBox(height: 18),
GestureDetector(onTap: () => Navigator.push(
context,
MaterialPageRoute(builder: (c) => const BanksPage())),
child: HubWidgets.buildWideCard('ОФИЦИАЛЬНЫЙ КУРС ЦБ РФ',
usdRate,
eurRate,
cnyRate,
...
const SizedBox(width: 18),
Expanded(child: HubWidgets.buildSmallCard('НОВОСТИ', 'ЯСИА', Icons.newspaper, const Color(0xFF007AFF),
        () async {
      final url = Uri.parse('https://ysia.ru');
...
HubWidgets.buildBigCard('АФИША ГОРОДА', 'Куда сходить сегодня?', Icons.confirmation_number_outlined, const Color(0xFFFF3B30),
        () async {
      final url = Uri.parse('https://afisha.ykt.ru');
      if (await canLaunchUrl(url)) await launchUrl(url);
    }),
